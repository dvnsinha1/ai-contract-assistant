...
app.post('/api/analyze-contract', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Processing ${chunks.length} chunks in background...`);
    
    // Start background processing without waiting
    processInBackground(chunks).catch(console.error);
...
async function processInBackground(chunks: string[]) {
  try {
    ...
    for (let i = 0; i < chunks.length; i += CONCURRENT_CHUNKS) {
      ...
      await Promise.all(chunkGroup.map(chunk => 
        model.generateContent(`
          Quick analysis of this section (2-3 sentences):
          ${chunk}
        `).catch(error => {
      ...
      console.log(`Processed chunks ${i + 1}-${i + chunkGroup.length} of ${chunks.length}`);
    }
  } catch (error) {
...
// Add endpoint to check analysis progress
app.get('/api/analyze-contract/status/:analysisId', (req: Request, res: Response) => {
  const { analysisId } = req.params;
  // In a real implementation, you would check the status of the analysis
